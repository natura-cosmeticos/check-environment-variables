describe('variableCheckers:StringVariableChecker', function () {
  const { expect } = require('chai')
  const StringVariableChecker = require('../StringVariableChecker')

  it('returns an error when variableValue does not match the regex', function () {
  ...
  it('returns an error when variableValue is shorter than the mininum length', function () {
  ...
  it('uses 1 as minLength when not set', function () {
    const { error } = StringVariableChecker('', { variableName: 'SHELL' })
    expect(error).to.equal('The value is shorter than 1 characters.')
  })

  it('uses 1 as mininumValue of minLength when the value set is less than 1', function () {
    const { error } = StringVariableChecker('', { variableName: 'SHELL', minLength: -1 })
    expect(error).to.equal('The value is shorter than 1 characters.')
  })

  it('returns an error when variableValue is longer than the maximum length', function () {
  ...
  it('uses 65536 as maxLength when not set', function () {
  ...
  it('uses 65536 as maximumValue of maxLength when the value set is less than 1', function () {
